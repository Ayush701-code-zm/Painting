...
export const metadata: Metadata = {
  title: "Collections",
  description:
    "Browse original paintings by style — Anime, Sketch, Abstract, and Contemporary.",
};

export default function CollectionsPage() {
  const collectionsWithWorks = COLLECTIONS.map((col) => {
    const works = ARTWORKS.filter(
      (a) => a.style.toLowerCase() === col.id.toLowerCase()
    );
    return { ...col, works, count: works.length, cover: works[0] };
          ...
          <FadeInSection>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-[1.25rem] pb-[5rem]">
              {collectionsWithWorks.map((col) => (
                ...
                >
                  <div className="aspect-[16/9] w-full relative bg-[#f0ead8]">
                    {col.cover?.image ? (
                      <Image
                        src={col.cover.image}
                        alt={col.cover.imageAlt ?? col.title}
                        fill
                    ...
                    <div className="absolute inset-0 bg-[linear-gradient(0deg,rgba(10,8,6,0.65)_0%,rgba(10,8,6,0.15)_60%)]" />
